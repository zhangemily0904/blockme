...
final class ListingRepoTests: XCTestCase {
  ...
  func testAddDeleteListing() throws {
    let newListing = Listing(seller: listingSeller, price: 10.0, expirationTime: Date.now + 100, availableLocations: [DiningLocation.LaPrimaWean])
    ...
    if result == XCTWaiter.Result.timedOut {
      ...
      XCTAssertTrue(listings.count == 1)
